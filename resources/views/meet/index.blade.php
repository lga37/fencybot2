...
<table class="table table-striped">
    ...
    <tr>
        ...
        <td>{{ is_array($meet->coords)? count($meet->coords) : 0 }}</td>
        <td>{{ $meet->coords  }}</td>
        <td>{{ $meet->dt }}</td>
        <td>{{ $meet->device->name ?? '' }}</td>
        <td>{{ $meet->lat }} {{ $meet->lng }} </td>

        <td class="px-4 py-4">
            <button class="btn btn-primary"
            data-cerca="{{ $meet->coords }}"
            data-lat="{{ $meet->lat }}"
                data-lng="{{ $meet->lng }}"
                data-toggle="modal" data-target="#modal">map</button>
        </td>
        <td class="px-4 py-4">
            <form method="POST" action="{{ route('meet.destroy',['meet'=>$meet]) }}">
                @method('DELETE')
                @csrf
